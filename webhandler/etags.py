...
def setEtag(tag, etag):
    if not os.path.isfile('downloads/etags.json'):
        print("No ETag file, initializing")
        with open('downloads/etags.json', 'w+', encoding="utf-8") as jfile: 
            newfile = {"created_with" : "lyfe_get"}
            json.dump(newfile, jfile, indent=4,)

    #open log
    newentry = {tag : etag}
    with open('downloads/etags.json', 'r', encoding="utf-8") as jfile:  
...
def getEtag(tag):
    try:
        with open('downloads/etags.json', 'r', encoding="utf-8") as json_file:  
